...
char formulatokenbuf[TOKENBUFLEN]; // 4-10 homework answer: save formula's all character using getallformulatoken;
int tokenbufidx = 0; // 4-10 homework answer: index of formulatokenbuf;
...
int gettokenbuf(); // 4-10 homework answer: get character from formulatokenbuf
void ungettokenbuf(); // 4-10 homework answer: minus 1 to tokenbufidx for get previous character when call gettokenbuf function;

int getop(char s[]);
void getallformulatoken(); // 4-10 homework answer: get all token of formulas using getline;
...
void ungettokenbuf() { // 4-10 homework answer: minus 1 to tokenbufidx for get previous character when call gettokenbuf function;
	if(tokenbufidx <= 0) {
		printf("ungettokenbuf: too many character\n");
	} else {
		tokenbufidx--;
	}
}
...
int getstrelement(char c) { // 4-5,4-6 homework answer: check if calculation's str elements in calculation formula;
	...
	strelement[i++] = c;
	while(((strelement[i++] = tolower((c = gettokenbuf()))) >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'));
	strelement[--i] = '\0';
	ungettokenbuf();
...
int getop(char s[]) {
	...
	i = 0;
	if(isdigit(c))
		while(isdigit(s[++i] = c = gettokenbuf()));
	if(c == '.')
		while(isdigit(s[++i] = c = gettokenbuf()));
	s[i] = '\0';
...
void getallformulatoken() { // 4-10 homework answer: get all token of formulas using getline;
	...
	while ((nread = getline(&line, &len, stdin)) != -1) {
        	for(j = 0; line[j] != '\0'; j++) {
			formulatokenbuf[i] = line[j];
			i++;
		}
        }
	formulatokenbuf[i] = UEOF;
...
int main() {
	...
	getallformulatoken(); // 4-10 homework answer: get all token of formulas using getline;
